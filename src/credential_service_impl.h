...
namespace One_data_share {
    ...
    public:
        /**
         * Creates a new Credential_service_impl object with the specified
         ...
         * REST API calls
         */
        Credential_service_impl(const std::string& ods_auth_token, const std::string& ods_url, std::unique_ptr<Rest> rest_caller);

        virtual std::string oauth_url(Oauth_endpoint_type type) const override;
        virtual void register_credential(Credential_endpoint_type type, const std::string& cred_id, const std::string& uri, const std::string& username, const std::string& secret) const override;
        virtual std::vector<std::string> credential_id_list(Endpoint_type type) const override;

        virtual ~Credential_service_impl() override;
    private:
        /**
